...
package com.zxing.qrcodelib.encode;
...
import com.zxing.qrcodelib.decode.Intents;
...
public final class QRCodeEncoder {
    ...
    private final static int CONTENTS_CONTACT = 1;

    private final static int CONTENTS_EMAIL = 2;

    private final static int CONTENTS_LOCATION = 3;

    private final static int CONTENTS_PHONE = 4;

    private final static int CONTENTS_SMS = 5;

    private final static int CONTENTS_TEXT = 6;
    ...
    public QRCodeEncoder(Context activity, EncodeParameters par, int dimension, boolean useVCard)
            throws WriterException {
        this.activity = activity;
        this.dimension = dimension;
        this.useVCard = useVCard;
        encodeContentsFromZXingIntent(par);
    }
    ...
    private boolean encodeContentsFromZXingIntent(EncodeParameters par) {
        // Default to QR_CODE if no format given.
    	String formatString = par.format;
        format = null;
        ...
        if (format == null || format == BarcodeFormat.QR_CODE) {
            ...
            encodeQRCodeContents(par, type);
        } else {
            String data = (String)par.data;
            if (data != null && !data.isEmpty()) {
                contents = data;
                displayContents = data;
                title = "Plain text";
                qRCodeType = CONTENTS_TEXT;
            }
        }
 ...
 // It would be nice if the string encoding lived in the core ZXing library,
    ...
    private boolean encodeContentsFromZXingIntent(Intent intent) {
      ...
      par.format = intent.getStringExtra(Intents.Encode.FORMAT);
      if (format == null || format == BarcodeFormat.QR_CODE) {
    	par.type = intent.getStringExtra(Intents.Encode.TYPE);
    	if(par.type.equals(Contents.Type.TEXT)){
    		par.data = intent.getStringExtra(Intents.Encode.DATA);
    	} else if(par.type.equals(Contents.Type.TEXT)
    			||par.type.equals(Contents.Type.EMAIL)
    			||par.type.equals(Contents.Type.PHONE)
    			||par.type.equals(Contents.Type.SMS)){
    		par.data = ContactEncoder.trim(intent.getStringExtra(Intents.Encode.DATA));
    	}else if(par.type.equals(Contents.Type.CONTACT)
    			||par.type.equals(Contents.Type.LOCATION)){
    		par.data = intent.getBundleExtra(Intents.Encode.DATA);;
    	}
      } else {
    	 par.data = intent.getStringExtra(Intents.Encode.DATA);
      }
      return encodeContentsFromZXingIntent(par);
    }
    ...
    private void encodeQRCodeContents(EncodeParameters par, String type) {
        ...
        if (Contents.Type.TEXT.equals(type)) {
            ...
            String data = ContactEncoder.trim((String)par.data);
            if (data != null) {
                contents = "mailto:" + data;
                displayContents = data;
                title = "Email address";
                qRCodeType = CONTENTS_EMAIL;
            }
            ...
            if (data != null) {
                ...
                displayContents = PhoneNumberUtils.formatNumber(data);
                title = "SMS address";
                qRCodeType = CONTENTS_SMS;
            }
        } else if (Contents.Type.CONTACT.equals(type)) {

            Bundle bundle = (Bundle)par.data;
            if (bundle != null) {
    ...
    private void encodeQRCodeContents(AddressBookParsedResult contact) {
        ...
        String[] encoded = encoder.encode(toList(contact.getNames()), contact.getOrg(),
                toList(contact.getAddresses()), toList(contact.getPhoneNumbers()), null,
